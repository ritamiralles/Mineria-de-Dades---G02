...
#Això es fa només amb el train, i després s’aplica igual al test(acaben tenint exctament les mateixes columes)dmy <- dummyVars(~ ., data = df_tr[, vars_x, drop = FALSE], fullRank = TRUE)
...
#BALANCEIG AMB UPSAMPLE

#La funció upSample() del paquet caret fa oversampling
#replica observacions de la classe minoritària (Exited = Yes)
# fins que tingui el mateix nombre que la classe majoritària.
...
svm_in_bal <- svm(
  ...
  data   = train_in_bal,    #utilitzem nomes 70% train
